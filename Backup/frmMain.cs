...
/*--------------------------------------------------------------------------*/
			...
			try
			{
				...
				if (IScope == null)
				{
					...
					if (progID == "")
					{
						...
						{
							MessageBox.Show("Cound not find \'AstrooptikServer.Telescope\'");
							return false;
						}
				...
				if (IScope != null) m_bScopeConnect = true;
			...
			try
			{
				...
				if (IFocus == null || IFocus.Link == false)
				{
					if (IFocus == null)
					{
						...
						if (progID == null || progID == "")
						{
							progID = Focuser.Choose("AstrooptikServer.Focuser");
							if (progID == null || progID == "") MessageBox.Show("Could not find \'AstrooptikServer.Focuser\'");
							else m_config.SetParamValue("Instrument", "Focuser ProgID", progID);
						}
						if (progID != null && progID != "") IFocus = new Focuser(progID);
					}
					if (IFocus != null) IFocus.Link = true;
		...
		{
			frmParamNet frame = new frmParamNet(this);
